...
const { widget } = figma
const { useSyncedState, usePropertyMenu, AutoLayout, Text, Rectangle, Input, Line } = widget

// Create a string type variable to store the access token
let pat: string = '';

async function getComments() {
  // const pat = figma.clientStorage.getAsync('pat');
  console.log("Get comments" + pat);
  const response = await fetch(`https://api.figma.com/v1/files/${figma.fileKey}/comments`,
    {
      method: 'GET',
      headers: {
        'x-FIGMA-TOKEN': pat
...
function Widget() {
  const [todoItems, setTodoItems] = useSyncedState('todoItems', ['Item 1', 'Item 2', 'Item 3']);
    ...
    >
      {
        <Text onClick={async e => {
          ...
          // console.log(res);
          // Extract out just the message from each comment to another array
          const messages = res.comments.map((comment: { message: string; }) => comment.message);
          console.log(messages);
          setTodoItems(messages);
        }}>Get File key</Text>
      }
      {
        <AutoLayout width={'fill-parent'} padding={8} fill="#000">
          {
            // Heading of the widget
            <Text fill={'#fff'} fontSize={16} fontWeight={600} hoverStyle={{ fill: '#fff' }} horizontalAlignText={'center'}>TODO</Text>
          }
      ...
      }
      {
        !pat && <Input
          ...
          onTextEditEnd={(e) => pat = e.characters}
          value={pat}
          width={'fill-parent'}
        />
      }
      ...
      {todoItems.map(item => {
        return (<AutoLayout
          ...
          <Text width={400} hoverStyle={{ fill: '#000' }} fontSize={16} horizontalAlignText={'left'}>{item}</Text>
        </AutoLayout>);
